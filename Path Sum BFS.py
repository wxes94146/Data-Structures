# 利用BFS找出tree中是否有指定路徑和之路徑
# input的第一行表示有幾個Nodes，接下來的每一行分別是Node編號、資料、與跟此node相連的node編號，最後一行會有個數字，請找尋數中是否存在一條由root到leaf的path其路徑上的和為此數字
# input:8
#       0 5 1,2
#       1 4 3
#       2 8 4,5
  ...
  def DFS_path_sum(self, node, goal, path_sum):
    ...
    for index in node.adjnodes:
      if index == -1:
        ...
        return True
    return False
  def BFS_path_sum(self, node, goal):
    P = deque([node])
    Q = deque([])
    while len(P) != 0:
      while len(P) != 0:
        ...
        if n.data == goal:
          return True
        for index in n.adjnodes:
          if index == -1:
            break
          self.graph_node[index].data = self.graph_node[index].data + n.data
          Q.append(self.graph_node[index])
      P,Q = Q,P
    return False
...
print(x.BFS_path_sum(x.graph_node[0], int(input())))